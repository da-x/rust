...
impl<'a, 'b> MacroExpander<'a, 'b> {
    ...
    fn expand(&mut self, expansion: Expansion) -> Expansion {
        ...
        let (expansion, mut invocations) = self.collect_invocations(expansion, &[], None);
        ...
        loop {
            ...
            let (expansion, new_invocations) = if let Some(ext) = ext {
                if let Some(ext) = ext {
                    ...
                    self.collect_invocations(expansion, derives, None)
                } else {
                    unreachable!()
                }
            } else {
    ...
    fn expand_invoc(&mut self, invoc: Invocation, ext: Rc<SyntaxExtension>) -> Option<(Expansion, Option<Path>)> {
            ...
            ast::ItemKind::Mac(..) => {
                ...
                item.and_then(|item| match item.node {
                    ItemKind::Mac(mac) => {
                        ...
                        self.collect(ExpansionKind::Items, bang).make_items()
                    }
                    _ => unreachable!(),
                })
            }
